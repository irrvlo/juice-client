...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const fetchAll = async () => {
    const [customizations, user] = await Promise.all([
      fetch("https://juice-api.irrvlo.xyz/api/customizations").then(r => r.json()),
    ...
    localStorage.setItem("current-user", JSON.stringify(user.statusCode === 401 ? {} : user))
  }
  fetchAll()
  ...
  const lobbyKeybindReminder = settings => {
    ...
    document.addEventListener("juice-settings-changed", ({ detail }) => {
      if (detail.setting === "menu_keybind") {
        ...
        if (keybindReminder) keybindReminder.innerText = `Press ${detail.value} to open the client menu.`
      }
    })
  };
  ...
  const applyUIFeatures = () => {
    ...
    const updateUIFeatures = () => {
      ...
      if (settings.perm_crosshair) styles.push(".crosshair-static { opacity: 1 !important; visibility: visible !important; display: block !important; }")
      ...
      if (settings.hitmarker_link !== "") styles.push(`.hitmark { content: url(${formatLink(settings.hitmarker_link)}) !important; }`)
      if (settings.killicon_link !== "") styles.push(`.animate-cont::before { content: ""; 
      ...
      if (settings.hide_keybind_reminder) styles.push("#juice-keybind-reminder { display: none; }")
  ...
  const handleLobby = () => {
    ...
    const applyCustomizations = () => {
      if (customizations?.find((c) => c.shortId === currentUser?.shortId)) {
        const customs = customizations.find(c => c.shortId === currentUser.shortId)
        const lobbyNickname = document.querySelector(".team-section .heads .nickname")

        if (customs.gradient) lobbyNickname.style = `
              display: flex; align-items: flex-end; gap: 0.25rem; overflow: unset !important;
              background: linear-gradient(${customs.gradient.rot}, ${customs.gradient.stops.join(", ")});
              -webkit-background-clip: text !important;
              -webkit-text-fill-color: transparent;
              text-shadow: ${customs.gradient.shadow || "0 0 0 transparent"} !important;
          `
        else lobbyNickname.style = "display: flex; align-items: flex-end; gap: 0.25rem; overflow: unset !important;";
    ...
    document.addEventListener("juice-settings-changed", ({ detail }) => {
      if (detail.setting === "customizations") detail.value ? applyCustomizations() : removeCustomizations()
    })
  ...
  const handleServers = async () => {
    const mapImages = await fetch("https://raw.githubusercontent.com/SheriffCarry/KirkaSkins/main/maps/full_mapimages.json")
      .then(res => res.json())
    ...
    let interval = setInterval(() => {
      if (!window.location.href.startsWith(`${base_url}servers/`)) clearInterval(interval)
      replaceMapImages()
    }, 250)

    document.addEventListener("click", e => {
      if (e.shiftKey && e.target.classList.contains("author-name")) setTimeout(() => {
        navigator.clipboard.readText().then(text => {
          window.location.href = `${base_url}profile/${text.replace("#", "")}`
          const username = e.target.innerText.replace(":", "");
          customNotification({ message: `Loading ${username}${text}'s profile...` })
        })
      }, 250)
    })
  }
  ...
  const handleProfile = () => {
    ...
    const interval = setInterval(() => {
      if (!window.location.href.startsWith(`${base_url}profile/`)) clearInterval(interval)

      if (document.querySelector(".profile > .content")) {
        clearInterval(interval)

        const profile = document.querySelector(".content > .profile-cont > .profile")
        ...
        if (progressExp) {
          ...
          progressExp.innerText = `${parseInt(current).toLocaleString()}/${parseInt(max).toLocaleString()}`
        }
        ...
        statistics.forEach(stat => {
          ...
          stat.querySelector(".stat-value").innerText = value.replace(value.split(" ")[0], parseInt(value.split(" ")[0]).toLocaleString())
        })

        const kloElem = content.querySelector(".card.k-d")
        const kloStat = kloElem.querySelector(".stat-value")
        kloStat.innerText = kloStat.innerText.replace(kloStat.innerText, parseInt(kloStat.innerText).toLocaleString())
        ...
        kloClone.querySelector(".stat-value").innerText = (parseFloat(kills) / parseFloat(deaths)).toFixed(2)
  ...
  const handleInGame = () => {
    ...
    const interval = setInterval(() => {
      if (!window.location.href.startsWith(`${base_url}games/`)) clearInterval(interval)
  ...
  const handleFriends = () => {
    ...
    const interval = setInterval(() => {
      if (!window.location.href.startsWith(`${base_url}friends`)) clearInterval(interval)
      ...
      if (settings.customizations) {
        ...
        friends.forEach(friend => {
          ...
          if (customs) {
            ...
            if (customs.gradient) nickname.style = `
              display: flex !important;
              align-items: flex-end !important;
              gap: 0.25rem !important;
            ...
            if (customs.badges && customs.badges.length) customs.badges.forEach(badge => {
              const img = document.createElement("img")
              img.src = badge
              img.style.cssText = badgeStyle
              badgesElem.appendChild(img)
