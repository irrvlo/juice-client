...
const customReqScripts = (base_url) => {
	  ...
	  for (let i = 0; i < itemElements.length; i++) {
		...
		try {
		  ...
		  let fetchreq = await fetch(
			`https://api.kirka.io/api/user/getProfile`,
			{
			  headers: {
	...
	const observer = new MutationObserver(() => {
	  if (window.location.href === `${base_url}inventory`) {
		const sellElem = document.querySelector(".cont-sell");
		if (sellElem && !document.getElementById("juice-custom-listing")) {
