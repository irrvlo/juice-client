...
const initResourceSwapper = () => {
  ...
  const SWAP_FOLDER = path.join(app.getPath("documents"), "JuiceClient", "swapper");
  ...
  const allFilesSync = (dir) => {
    fs.readdirSync(dir).forEach((file) => {
      ...
      if (fs.statSync(filePath).isDirectory()) allFilesSync(filePath);
        ...
        const useAssets = /JuiceClient[\\/]swapper[\\/]assets[\\/](css|media|img|glb)[\\/].*\.(.{4})/.test(filePath);
        if (!useAssets) return;
  ...
  if (swap.filter.urls.length) {
    session.defaultSession.webRequest.onBeforeRequest(
      swap.filter,
      (details, callback) => {
        const redirect = "juiceclient://" + (swap.files[details.url.replace(/https|http|(\?.*)|(#.*)/gi, "")] || details.url);
